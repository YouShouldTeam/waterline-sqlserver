...
var sql = {
  ...
  escape: function (val, updateOrInsert) {
    if (val === undefined || val === null) {
      return 'NULL';
    }

    if (updateOrInsert === undefined || updateOrInsert === null) {
    ...
    // cast to nvarchar to avoid issue with encoding strange char like viet stuff
    if(updateOrInsert) {
      return "CONVERT(nvarchar(max), N'" + val +"')";
    }
  ...
  prepareCriterion: function (collectionName, value, key, parentKey) {
    ...
    if (parentKey) {
      ...
      valueStr = sql.prepareValue(collectionName, value);
      ...
      valueStr = sql.prepareValue(collectionName, value, true);
  ...
  prepareValue: function (collectionName, value, updateOrInsert) {
    // Cast dates to SQL
    if (_.isDate(value)) {
    ...
    return sql.escape(value, updateOrInsert);
  },
  ...
  predicate: function (collectionName, criterion, key, parentKey) {
    ...
    // LIKE
      ...
      return sql.build(collectionName, criterion, function (collectionName, value, attrName) {
        ...
        var valueStr = sql.prepareValue(collectionName, value);
